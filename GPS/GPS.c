...
void get_actual_checksum (char check_sum[],  char gprmc_str[]){
    // last two characters im gprmc_str are the checksum
...
bool valid_checksum (char gprmc_str[]){
	...
	char checksum[2];
    get_actual_checksum(checksum, gprmc_str);
    actual_checksum = strtol(checksum, NULL, 16); // convert checksum from hex to int
    ...
    while (token != NULL) {
        ...
        case 3: // Latitude
            *latitude = convert_to_degrees(atof(token));
						//*latitude=floor(1000*(*latitude))/1000;
            break;
        ...
        case 5: // Longitude
            *longitude = convert_to_degrees(atof(token));
						//*longitude=floor(1000*(*longitude))/1000;
            break;
...
double calculate_distance(double lat1, double lon1, double lat2, double lon2) {
    ...
    // in cese of error(not moving) , return 0 
	if(d <= 0.5){
		return 0;
	}
