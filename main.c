...
int main(void){
    ...
    UART7Init();
      ...
      UART7_readstr(message);
        ...
        if(currentLat == 0 && currentLong == 0) continue;
        else break;
    }
    ...
    while(1){
        ...
        parse_gprmc(message, &currentLat, &currentLong);
			...
			//delay_IN_ms(1500);
				...
				strcat(lat_str , ",");
				strcat(lon_str, ",");
				//sprintf(prevLat_str, "%f" ,prevLat);
				//strcat(prevLat_str,",");
				//sprintf(prevLon_str, "%f" ,prevLong);
				//strcat(prevLon_str,">");
				UART0_printf(lat_str);
				UART0_printf(lon_str);
				//UART0_printf(prevLat_str);
				//UART0_printf(prevLon_str);
        //if the GPS message is not valid skip iteration
        ...
        distanceCovered +=  calculate_distance(currentLat, currentLong, prevLat, prevLong);
				//strcat(lat_str , ",");
				//strcat(lon_str, ",");
				//sprintf(prevLat_str, "%0.3f" ,prevLat);
				//strcat(prevLat_str,",");
				//sprintf(prevLon_str, "%0.3f" ,prevLong);
				//strcat(prevLon_str,">");
				//UART0_printf(lat_str);
				//UART0_printf(lon_str);
				//UART0_printf(prevLat_str);
